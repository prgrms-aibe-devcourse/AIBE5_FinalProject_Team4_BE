...
| `MYPAGE-001`~`MYPAGE-005` | 마이페이지 | 프로필/계정 관리 | 내 정보 조회/수정, 소셜 계정 확인, 룩피드 프로필 편집, 로그아웃, 회원 탈퇴 | `USERS`, `SOCIAL_ACCOUNTS`, `USER_EXTERNAL_LINK`, `USER_STYLES` | 마이페이지, 룩피드 프로필 |
...
| `EXT-003`~`EXT-005` | 외부 연동 | 날씨 보조 정보 | 사용자 지역 기반 현재 날씨와 체감온도 조회. 독립 추천 기능이 아니라 추천 보조 조건 | `GET /api/weather` | 추천 메인 |
| `EXT-006`~`EXT-007` | 외부 연동 | AI 의류 분석/추천 설명 | 옷 사진 분석, 스타일링 설명 또는 추천 이유 생성 | Gemini API | 옷 등록, 추천 상세 |
