...
| `SYSTEM-006`~`SYSTEM-007` | 공통 시스템 | 서비스 안내 | 서비스 소개와 도움말/가이드 투어 안내. 페이지별 투어 완료 상태를 BE에서 관리하며 로그인 후 미완료 시 자동 노출 | `GET /api/v1/users/profile` (guideTourCompleted* 필드), `PATCH /api/v1/users/guide-tour`, `USERS.guide_tour_completed_home/wardrobe/feed/mypage`, `docs/planning/project-plan.md`, `docs/legal/terms.md` | 홈, 옷장, 피드, 마이페이지 |
