...
| `season` | Y | 옷 자체의 대상 계절 code (`SPRING`, `SUMMER`, `FALL`, `WINTER`, `ALL_SEASON`). 옷 등록 시 1개 선택하며 생성 후 변경하지 않음 |
...
| 필드 | 필수 | 설명 |
| --- | --- | --- |
| `name`, `brandName`, `productCode`, `category`, `itemType`, `season`, `gender`, `primaryColor`, `styles`, `externalSource`, `size`, `favorite`, `isVerified` | Y | 옷 공통·옷장 정보 |
| `secondaryColors` | N | 보조 색상 |
