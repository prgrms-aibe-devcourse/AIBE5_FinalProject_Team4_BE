...
- `season`은 `CLOTHES.season` 기준으로 저장하며 생성 후 변경하지 않습니다. 현재 코드가 `WARDROBE_CLOTHES.season`을 수정하거나 저장하면 [implementation-gaps.md](../backend/implementation-gaps.md)에서 정합성을 확인합니다.
