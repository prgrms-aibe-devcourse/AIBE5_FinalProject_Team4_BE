...
**예외 — `RECO-005` 어울리는 옷 추천**: 같은 어울림 점수(`compatibilityScore`) 그룹 안에서는 `brandName`이 `UNKNOWN`이 아닌 후보를 먼저 노출합니다. 두 후보 모두 브랜드가 알려져 있거나 모두 `UNKNOWN`이면 조회 순서에 따릅니다.

이 정책은 같은 사용자에게도 같은 점수 후보의 순서가 매번 달라질 수 있음을 의미합니다(`RECO-005` 제외). FE는 같은 점수 후보의 순서 변경을 오류로 보지 않습니다.
...
`RECO-006` AI MD 상품 추천은 내부 `EXTERNAL_SHOPPING` 공용 후보를 우선 사용합니다.
...
- 같은 점수 그룹 안에서는 랜덤 노출될 수 있습니다. `RECO-005` 어울리는 옷 추천은 동점 시 `UNKNOWN`이 아닌 브랜드를 우선합니다.
...
- 점수 동점 그룹 랜덤 노출은 추천 품질을 해치지 않는 범위에서 수행합니다. `RECO-005`는 동점 시 `brandName != UNKNOWN` 우선 정렬을 적용합니다.
- `RECO-005` `limitPerCategory` query는 기본 `5`, 허용 범위 `1`~`50`입니다. 컨트롤러 validation과 [api-contract.md](../api/api-contract.md)를 동일하게 유지합니다.
