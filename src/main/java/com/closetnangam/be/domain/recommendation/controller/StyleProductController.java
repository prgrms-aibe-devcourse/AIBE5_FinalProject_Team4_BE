...
public class StyleProductController {
    ...
    public ResponseEntity<ApiResponse<List<RecommendResponse>>> getRecommendations(
            @PathVariable Long wardrobeId,
            @RequestParam(defaultValue = "20.0") double currentTemp) {
        Long currentUserId = SecurityUtils.getCurrentUserId();
        return ResponseEntity.ok(ApiResponse.ok(styleProductRecommender.recommendByStyle(currentUserId, wardrobeId, currentTemp)));
    }
}
